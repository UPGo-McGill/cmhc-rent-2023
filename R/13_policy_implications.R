...
did_effects <-
  tibble(
    treat = md$main$rent_log$group,
    year = md$main$rent_log$t,
    att = md$main$rent_log$att
  ) |>
  filter(year >= treat)

# Main DiD rent difference
did_rent_dif <-
  dd$main |>
  ...
  mutate(
    rent_log_cf_raw = rent_log_cf *
      sd(dr$main$rent_log_raw) +
...
# Spillover DiD effects
did_adj_rent_effects <-
  tibble(
    treat = md$adj$rent_log$group,
    year = md$adj$rent_log$t,
    att = md$adj$rent_log$att
  ) |>
  filter(year >= treat)

# Spillover DiD rent difference
did_adj_rent_dif <-
  dd$adj |>
  mutate(id = as.character(id)) |>
  # Add effects to data
  inner_join(did_adj_effects, by = c("year", "treat")) |>
  # Create counterfactual for rent_log in the absence of treatment
  mutate(rent_log_cf = rent_log - att) |>
  # Un-standardize rent_log_cf, using dr$main to get complete data range
  mutate(
    rent_log_cf_raw = rent_log_cf *
      sd(dr$main$rent_log_raw) +
      mean(dr$main$rent_log_raw)
...
# Main DiD FREH effects
did_FREH_effects <-
  tibble(
    treat = md$main$FREH$group,
    year = md$main$FREH$t,
    att = md$main$FREH$att
  ) |>
  filter(year >= treat)

# Main DiD FREH difference
did_FREH_dif <-
  dd$main |>
  mutate(id = as.character(id)) |>
  ...
  mutate(
    FREH_cf_raw = FREH_cf *
      sd(dr$main$FREH_raw, na.rm = TRUE) +
...
# Main DiD non-FREH effects
did_non_FREH_effects <-
  tibble(
    treat = md$main$non_FREH$group,
    year = md$main$non_FREH$t,
    att = md$main$non_FREH$att
  ) |>
  filter(year >= treat)

# Main DiD non-FREH difference
did_non_FREH_dif <-
  dd$main |>
  mutate(id = as.character(id)) |>
...
# Spillover DiD FREH effects
did_adj_FREH_effects <-
  tibble(
    treat = md$adj$FREH$group,
    year = md$adj$FREH$t,
    att = md$adj$FREH$att
  ) |>
  filter(year >= treat)

# Spillover DiD FREH difference
did_adj_FREH_dif <-
  dd$adj |>
  mutate(id = as.character(id)) |>
  inner_join(did_adj_FREH_effects, by = c("year", "treat")) |>
  mutate(FREH_cf = FREH - att) |>
  mutate(
    FREH_cf_raw = FREH_cf *
      sd(dr$main$FREH_raw, na.rm = TRUE) +
...
# Spillover DiD non-FREH effects
did_adj_non_FREH_effects <-
  tibble(
    treat = md$adj$non_FREH$group,
    year = md$adj$non_FREH$t,
    att = md$adj$non_FREH$att
  ) |>
  filter(year >= treat)

# Spillover DiD non-FREH difference
did_adj_non_FREH_dif <-
  dd$adj |>
  mutate(id = as.character(id)) |>
  inner_join(did_adj_non_FREH_effects, by = c("year", "treat")) |>
  mutate(non_FREH_cf = non_FREH - att) |>
  mutate(
    non_FREH_cf_raw = non_FREH_cf *
      sd(dr$main$non_FREH_raw, na.rm = TRUE) +
...
did_rent_dif |>
  filter(year == treat) |>
  mutate(
    total_rent = rent_raw * tenant_count,
  ...
  mutate(
    pct = scales::percent(
      total_rent_dif / (total_rent + total_rent_dif),
      0.1
    )
  )
...
did_FREH_dif |>
  filter(year == treat) |>
  mutate(
    total_FREH = FREH_raw * tenant_count,
  ...
  mutate(
    pct = scales::percent(
      total_FREH_dif / (total_FREH + total_FREH_dif),
      0.1
    )
  )
...
did_non_FREH_dif |>
  filter(year == treat) |>
  mutate(
    total_non_FREH = non_FREH_raw * tenant_count,
  ...
  mutate(
    pct = scales::percent(
      total_non_FREH_dif / (total_non_FREH + total_non_FREH_dif),
      0.1
    )
  )


# DiD 2023 implications ---------------------------------------------------
...
# Average spillover rent decrease in 2023
did_adj_rent_2023 <-
  did_adj_rent_dif |>
  filter(year == 2023) |>
  mutate(
...
did_adj_rent_2023
scales::dollar(
  did_rent_2023$total_rent_dif + did_adj_rent_2023$total_rent_dif,
  100000,
  big.mark = ","
)
...
did_rent_non_treated <-
  ...
  anti_join(did_adj_rent_dif, by = c("id")) |>
  filter(year == 2023) |>
...
back_to_2017 <-
  map(2018:2022, \(x) {
    dc$main |>
      ...
      mutate(
        FREH_change_new = FREH_2017 - FREH_lag_raw,
...
fig_6 <-
  ...
  mutate(
    type = factor(type, levels = c("Year-to-year effect", "Cumulative effect"))
  ) |>
  ...
  geom_label(
    aes(
      label = scales::dollar(str_share, 0.1, scale = 1 / 1000000, suffix = "M")
    ),
    family = "Futura",
    nudge_y = c(
      -0.015,
...
prov_FREH <-
  ...
  filter(!is.na(region)) |>
  summarize(
    across(total_before:str_share, sum),
    str_pct = str_share / total_change,
    .by = region
  ) |>
  mutate(
    total_change = scales::dollar(
      total_change,
      0.1,
      scale = 1 / 1000000,
      suffix = "M"
    ),
    str_share = scales::dollar(
      str_share,
      0.1,
      scale = 1 / 1000000,
      suffix = "M"
