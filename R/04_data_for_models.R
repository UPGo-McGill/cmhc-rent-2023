...
data_rent <-
  ...
  mutate(across(c(rent, FREH_share, rev_share, universe, pop_CMA, 
                  accommodation), .fns = list(log = log), 
                .names = "{.fn}_{.col}")) |> 
  select(id, year, CMA, province, rent, log_rent, FREH_share, log_FREH_share, 
         rev_share, log_rev_share, universe, log_universe, tenant_share, 
         pop_CMA, log_pop_CMA, accommodation, log_accommodation) |> 
  filter(!is.infinite(log_FREH_share), !is.infinite(log_rev_share),
         !is.infinite(log_universe), !is.infinite(log_pop_CMA), 
         !is.infinite(log_accommodation)) |> 
  na.omit()
...
data_change <- 
  ...
  mutate(year = as.character(year)) |> 
  mutate(across(c(universe, pop_CMA, accommodation), 
                .fns = list(log = log), .names = "{.fn}_{.col}")) |> 
  select(id, year, CMA, province, rent_change_pct, FREH_change_pct, FREH_share,
         rev_change_pct, rev_share, universe, log_universe, tenant_share, 
         pop_CMA, log_pop_CMA, accommodation, log_accommodation) |> 
  filter(!is.infinite(log_universe), !is.infinite(log_pop_CMA), 
         !is.infinite(log_accommodation)) |> 
  na.omit()
