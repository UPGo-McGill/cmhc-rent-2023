...
# province <-
#   get_census("CA21", regions = list(C = 1), level = "PR", geo_format = "sf") |>
#   st_transform(4326) |>
#   select(province = name, province_ID = GeoUID, geometry) |>
#   mutate(province = str_remove(province, " \\(.*\\)"))

# CMA <-
#   get_census("CA21", regions = list(C = 1), level = "CMA", geo_format = "sf") |>
#   st_transform(3347) |>
#   select(
#     CMA = GeoUID,
#     name_CMA = name,
#     pop_CMA = Population,
#     dwellings_CMA = Dwellings,
#     geometry
#   ) |>
...
#     CMA = CMA_UID,
#     pop_CSD = Population,
#     households_CSD = Households,
#     dwellings_CSD = Dwellings,
#     rent:income_parent,
#     geometry
#   ) |>
...
#   CSD |>
#   mutate(
#     tourism = (entertainment + accommodation) / tourism_parent,
#     .after = dwellings_CSD
#   ) |>
#   mutate(tenant = tenants / (tenants + owners), .after = tenants) |>
#   rename(tenant_count = tenants) |>
#   mutate(
#     apart = (apart_small + apart_big) / apart_parent,
#     .after = apart_parent
#   ) |>
#   select(
#     -c(
...
cmhc_int <-
  ...
  st_set_agr("constant") |>
  st_intersection(st_set_agr(
    mutate(
      cmhc_nbhd,
      area_2022 = as.numeric(
        st_area(geometry)
      )
    ),
    "constant"
...
cmhc_nbhd <-
  cmhc_nbhd |>
  mutate(
    area_CMHC = units::drop_units(st_area(geometry)),
    .before = geometry
  ) |>
  st_set_agr("constant")
...
cmhc_CMA <-
  ...
  st_centroid() |>
  mutate(
    CMA_id = unlist(nngeo::st_nn(geometry, CMA)),
...
cmhc_2016 <-
  ...
  select(
    ...
    universe,
    rent = average_rent,
    rent_rel = average_rent_reliability,
    vacancy = vacancy_rate_percent,
...
cmhc_2015_unmatched <-
  ...
  add_row(
    name = c(
      ...
      "Douglas/Bright",
      "Devon",
      "North-East Fredericton",
      "South-East Fredericton",
      "Lincoln/New Maryland/Gladstone"
    ),
    CMA = "13320",
    year = 2015,
  ...
  mutate(
    name = case_when(
      # CMA_all[4]
      name ==
        "Grand Bay/Westfield/Greenwich/Kingston" ~ "Eastern Saint John CMA",
      # CMA_all[7]
      ...
      name ==
        "Parish of L'Epiphanie/Ville de L'Epiphanie/Saint-Gerard-Majella" ~
      ...
      name ==
        paste0(
          "Vaudreuil-Dorion/Vaudreuil-sur-le-Lac/",
      ...
      name ==
        paste0(
          "West of Otonabee River (Smith-Ennismore-",
      ...
      name ==
        paste0(
          "Henderson Lake/Glendale/Sherring/",
          "Victoria Park/Shakleford Industrial Parks"
        ) ~ "North-East",
      name == "Indian Battle Hts/The Crossings/West Highlands" ~ "West-North",
      name == "Legacy Ridge/Uplands/Hardieville" ~ "North-West",
      name ==
        paste0(
          "Paradise Canyon/River Stone/Mountain Heights/Copperwood/",
...
cmhc_2023_matched <-
  cmhc_2023 |>
  inner_join(
    select(st_drop_geometry(cmhc_nbhd), id, name, CMA),
    by = c("name", "CMA")
  ) |>
  relocate(id)
...
cmhc_csd <-
  ...
  arrange(GeoUID, year) |>
  mutate(
    id = paste0(METCODE, "00", seq_along(GeoUID)),
    .by = c(METCODE, year),
    .before = GeoUID
  )
