...
monthly_sept |>
  mutate(across(c(FREH_change, non_FREH_change, price_change), \(x) {
    if_else(year <= 2018, NA, x)
  })) |>
  select(
    rent,
    FREH,
  ...
  st_drop_geometry() |>
  mutate(across(c(rent, FREH, non_FREH, price, vacancy_lag), \(x) {
  ...
  mutate(
    description = case_when(
      name ==
        "rent_log" ~ "Average monthly rent of purpose-built rentals (log)",
      ...
      name ==
        "FREH_change" ~ "Y.o.y. change in FREH listings as % of dwellings",
  ...
  bind_rows(
    x1 = dr$main |>
      ...
      summarize(
        name = "treated",
        description = "Was a PR restriction in effect on Jan 1 of the year?",
        source = "Authors",
        n = scales::comma(n()),
        mean = scales::comma(mean(treat != 0), 0.001),
        sd = "-",
        range = "-"
      ),
    x2 = _
...
ggsave("figures/figure_3.png", fig_3, width = 8, height = 4, units = "in")
