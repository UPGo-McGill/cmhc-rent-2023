...
#   strr_as_sf(3347) |>
#   mutate(
#     prov = unlist(nngeo::st_nn(
#       geometry,
#       st_transform(province, 3347)
#     ))
#   ) |>
...
#   monthly |>
#   inner_join(
#     mutate(exchange_rates, month = yearmonth(year_month)),
#     by = "month"
#   ) |>
...
#   arrange(property_ID, month) |>
#   mutate(
#     FREH = listing_type == "Entire home/apt" &
#       slide2_lgl(
#         A,
#         R,
#         \(A, R) sum(A) + sum(R) >= 183 & sum(R) > 90,
#         .before = 11
#       ),
#     FREH_60 = listing_type == "Entire home/apt" &
#       slide2_lgl(
#         A,
#         R,
#         \(A, R) sum(A) + sum(R) >= 120 & sum(R) > 60,
#         .before = 11
#       ),
#     FREH_120 = listing_type == "Entire home/apt" &
#       slide2_lgl(
#         A,
#         R,
#         \(A, R) sum(A) + sum(R) >= 240 & sum(R) > 120,
#         .before = 11
#       ),
#     .by = property_ID,
#     .after = B
...
#   mutate(
#     non_FREH = !FREH & A + R > 0,
#     non_FREH_60 = !FREH_60 & A + R > 0,
#     non_FREH_120 = !FREH_120 & A + R > 0,
#     .after = FREH_120
#   )
...
#   inner_join(
#     st_drop_geometry(
#       select(cmhc_nbhd, id, province, CMA, name_CMA)
#     ),
#     by = "id"
#   )
...
monthly_sept <-
  ...
  summarize(
    ...
    rev_count = sum(rev),
    FREH_count = sum(FREH),
    FREH_60_count = sum(FREH_60),
    FREH_120_count = sum(FREH_120),
    non_FREH_count = sum(non_FREH),
    non_FREH_60_count = sum(non_FREH_60),
    non_FREH_120_count = sum(non_FREH_120),
    price = sum(rev) / sum(R),
    .by = c(id, year)
  ) |>
  arrange(id, year) |>
  ...
  mutate(
    ...
    FREH_60 = FREH_60_count / dwellings,
    FREH_120 = FREH_120_count / dwellings,
    non_FREH = non_FREH_count / dwellings,
    non_FREH_60 = non_FREH_60_count / dwellings,
    non_FREH_120 = non_FREH_120_count / dwellings,
    STR = FREH + non_FREH,
    .after = price
  ) |>
  ...
  mutate(across(c(active_count:STR, active_count_change:STR_change), \(x) {
    if_else(year == "2023", NA, x)
  }))
