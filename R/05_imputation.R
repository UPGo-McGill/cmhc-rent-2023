...
monthly_impute <-
  monthly_impute |>
  mutate(
    rent_nn = if_else(id %in% special_cases, rent, rent_nn),
...
leftovers <-
  ...
  mutate(
    rent_lm = list(lm(rent ~ year, data = tibble(rent, year))),
    .after = rent_ratio,
    .by = id
  ) |>
  rowwise() |>
...
monthly_impute_vac <-
  ...
  mutate(
    vac_nn_group = mean(vac_nn, na.rm = TRUE),
    .by = id,
    .after = vac_nn
  ) |>
  mutate(
  ...
  rowwise() |>
  mutate(
    new_ratio = if (is.logical(vac_lm[[1]])) {
      NA_real_
    } else {
      predict(vac_lm, tibble(vac_ratio, year))
    },
    .after = vac_ratio
  ) |>
  ungroup() |>
