...
cpi <-
  ...
  select(
    REF_DATE,
    GEO,
    product = `Products and product groups`,
    UOM,
    value = VALUE
  ) |>
  filter(
    product %in% c("All-items", "Shelter"),
    GEO == "Canada",
    UOM == "2002=100"
  ) |>
...
fig_1a <-
  ...
  geom_line() +
  scale_linewidth_manual(values = c("TRUE" = 2, "FALSE" = 1), guide = NULL) +
...
cmhc |>
  inner_join(st_drop_geometry(select(cmhc_nbhd, id, province, CMA))) |>
  summarize(
    rent = sum(rent * universe, na.rm = TRUE) /
...
cpi |>
  filter(
    month %in% c(yearmonth("2015-10"), yearmonth("2023-10")),
    product == "All-items"
  ) |>
...
# Active/FREH listings by region
fig_1b <-
  monthly |>
  ...
  rename(`Active listings` = active, `FREH listings` = FREH) |>
...
monthly |>
  ...
  summarize(active = sum(A + R) / 30, FREH = sum(FREH))

# Change from pre-pandemic peak
monthly |>
  summarize(active = sum(A + R) / 30, FREH = sum(FREH), .by = month) |>
  summarize(
    active = active[month == max(month)] /
      max(active[month < yearmonth("2020-03")]) -
      1,
    FREH = FREH[month == max(month)] /
      max(FREH[month < yearmonth("2020-03")]) -
      1
  )
...
fig_1c <-
  monthly |>
  inner_join(
    qread("output/prop_csd.qs", nthreads = availableCores()),
    by = "property_ID"
  ) |>
  ...
  mutate(
    active_households = active / households_CSD,
    active_dwellings = active / dwellings_CSD
  ) |>
  mutate(
    dwell_size = case_when(
  ...
  summarize(
    active_dwellings = mean(active_dwellings, na.rm = TRUE),
  ...
  scale_x_yearmonth(name = NULL) +
  scale_y_continuous(
    name = "Active STRs as % of all dwellings",
