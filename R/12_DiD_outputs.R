...
ggsave("figures/figure_4.png", fig_4, width = 8, height = 4, units = "in")
...
fig_5 <-
  ...
  bind_rows(tibble(
    year = 2019:2022,
    att = cal$non_FREH$att.egt,
    SE = cal$non_FREH$se.egt,
    var = "non_FREH_log",
    model = "Main"
  )) |>
  bind_rows(tibble(
    year = 2019:2022,
    att = cal$van_non_FREH$att.egt,
  ...
  mutate(
    var = factor(
      var,
      levels = c("rent_log", "FREH_log", "non_FREH_log", "price_log")
    ),
...
ggsave("figures/figure_5.png", fig_5, width = 8, height = 4, units = "in")


# Figure 6: Spillover parallel trends assumption -------------------------

fig_6 <-
  aggte(md$adj$rent_log, type = "dynamic") |>
  ggdid() +
  ggtitle(NULL) +
  theme_minimal() +
  scale_x_continuous(
...
ggsave("figures/figure_6.png", fig_6, width = 8, height = 4, units = "in")


# Table 4: Parameter estimates for variant models ------------------------

tibble(
  var = names(ad$adj),
  att = round(map_dbl(ad$adj, \(x) x$overall.att), 3),
  se = round(map_dbl(ad$adj, \(x) x$overall.se), 3)
)

ad$adj

# Maximum confidence intervals
aggte(md$adj$rent_log, type = "simple", na.rm = TRUE, alp = .1)
aggte(md$adj$FREH, type = "simple", na.rm = TRUE, alp = .001)
