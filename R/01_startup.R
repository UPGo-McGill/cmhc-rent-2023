...
impute <- function(x) {
  x |>
    inner_join(
      monthly_impute,
      by = c("id", "year", "rent", "universe", "vacancy")
    ) |>
    mutate(
      rent = coalesce(rent, rent_new),
...
model_change <- function(
  ...
  x1 <-
    ...
    mutate(
      FREH_change_c = (FREH_change_c - mean(x$FREH_change_raw)) /
  ...
  x4 <-
    ...
    mutate(
      ...
      rent_change_raw_c = rent_change_c *
        sd(x$rent_change_raw) +
    ...
    mutate(
      total_before = rent_lag_raw * tenant_count_lag,
